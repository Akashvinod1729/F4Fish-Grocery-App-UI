...
class ProductController extends GetxController {
  ...
  void changeTotal() {
    totalAmount.value =
        ((expandResponse.value.data?.quantityType.first.sizeVariant?.price ?? 0) *
            quantityCount.value) as double;
  }
  ...
  final orderCreateResponse = ResponseClassify.error("").obs;
  ...
  orderProducts(OrderCreateModel model) {
    print(model.toJson());
    orderCreateResponse.value = ResponseClassify.loading();
    try {
      orderCreateResponse.value =
          ResponseClassify.completed(orderCreateUseCase.call(model));
    } catch (e) {
